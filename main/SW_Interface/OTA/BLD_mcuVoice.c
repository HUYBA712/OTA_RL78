...
bool bldVoice_binDataReadFromPartition(size_t offset_ra, void* dst, size_t size)
{
    ...
    return true;
#else
    if((offset_ra > 0x7FFFF) || (((uint32_t)offset_ra + size) > 524288)) return false;
...
bool bldMcuVoice_receiveOk(uint8_t* bufout, uint16_t bufsize, uint16_t* lenCoreBslOut, uint16_t lenExpect)
{
    ...
    if(bufout[0] != 0x81 || bufout[len-1] != 0x03) return false;
    uint8_t cks = bldMcuVoice_checksum(bufout+1, len - 3);
    cks = cks + bufout[len - 2];
    if(cks != 0) return false;
...
bool bldMcuVoice_inquiry()
{
    uint8_t fr[6] = {0x01, 0x00, 0x01, 0x00, 0xFF, 0x03};
    bldMcuVoice_sendMcuFrame((uint8_t*)fr, 6);
    uint8_t buf[7] = {0xFF};
    uint16_t lenCore;
    if(bldMcuVoice_receiveOk((uint8_t*)buf,7,&lenCore, 7) == false) return false;
    if(buf[3] == 0x00 && buf[4] == 0x00) return true;
    else return false;
}

bool bldMcuVoice_changeBaud(uint32_t baud)
{
    uint8_t fr[10] = {0x01, 0x00, 0x05, 0x34, 0x00,0x00,0x25,0x80, 0x22, 0x03};
    fr[4] = baud>>24;
    fr[5] = baud>>16;
    fr[6] = baud>>8;
    fr[7] = baud;
    fr[8] = 0 - bldMcuVoice_checksum(&fr[1],7);
    bldMcuVoice_sendMcuFrame((uint8_t*)fr, 10);
    uint8_t buf[7] = {0xFF};
    uint16_t lenCore;
    if(bldMcuVoice_receiveOk((uint8_t*)buf,7,&lenCore, 7) == false) return false;
    if(buf[3] == 0x34 && buf[4] == 0x00) return true;
    else return false;
}
bool bldMcuVoice_getSignature()
{
    uint8_t fr[6] = {0x01, 0x00, 0x01, 0x3A, 0xC5,0x03};
    bldMcuVoice_sendMcuFrame((uint8_t*)fr, 6);
    uint8_t buf[18] = {0xFF};
    uint16_t lenCore;
    if(bldMcuVoice_receiveOk((uint8_t*)buf,18,&lenCore, 18) == false) return false;
    if(buf[3] == 0x3A) {
        log_buf_mcuRsp(buf, 18);
        return true;
    }
    else return false;
}
...
bool bldMcuVoice_commandSendDataBlock(uint8_t* data, uint16_t len, uint32_t addr)
{
...
bool bldMcuVoice_commandSendData()
{
    ...
    uint8_t bufData[1024];       //trong info areas dang thay MAX = 128 byte
...
bool bldMcuVoice_checkData()
{
    ...
    uint32_t numOfUnit = 512;
    uint8_t bufSend[1024] = {0xFF};
    uint8_t buf[1024+6] = {0xFF};
...
static void bldMcu_task3(void *prg)
{
    ...
    {       
        ...
        if(bldMcuVoice_changeBaud(115200) == true)
        {
            vTaskDelay(3/portTICK_PERIOD_MS);
        ...
        if(bldMcuVoice_getSignature() == true)
        {
            printf("get signature ok\n");
        }
        else {
            bldMcuVoice_updateStatusMqtt(BLD_VOICE_STT_INQUIRY_FAIL);
            printf("get signature fail\n");
            BootModeRelease(NG);
            goto ota_done1;
        }
        if(bldMcuVoice_getAreaInfo() == true)
        {
            printf("get info areas ok\n");
        }
        else {
            bldMcuVoice_updateStatusMqtt(BLD_VOICE_STT_INQUIRY_FAIL);
            printf("get info areas fail\n");
            BootModeRelease(NG);
            goto ota_done1;
        }
        if(bldMcuVoice_eraseCommand() == true)
        {
            printf("erase ok\n");
        }
        else {
            bldMcuVoice_updateStatusMqtt(BLD_VOICE_STT_ERASE_FAIL);
            printf("erase fail\n");
            BootModeRelease(NG);
            goto ota_done1;
        }
